...
const snippets = {
  Predict: codeSnippet,
  Train: `from libreyolo import LibreYOLO\n\nmodel = LibreYOLO("LibreYOLO9t.pt")\nmodel.train(data="dataset.yaml", epochs=100)`,
  Export: `from libreyolo import LibreYOLO\n\nmodel = LibreYOLO("LibreYOLO9t.pt")\nmodel.export(format="onnx")`,
}

export function HeroSection({ quickstartTabs = false }) {
  const t = useTranslations('Home')
  const tc = useTranslations('Common')
  const [copied, setCopied] = useState(false)
  const [selected, setSelected] = useState('Predict')
  const activeSnippet = quickstartTabs ? snippets[selected] : codeSnippet
    ...
    <section className="relative md:min-h-screen md:flex md:items-center md:justify-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-5 md:px-6 lg:px-8 pt-24 pb-12 md:pt-32 md:pb-20">
        ...
        >
          ...
          >
            {quickstartTabs && 'LibreYOLO, the YOLO library. '}{t('heroSubA')}
                ...
                <div className="relative code-block rounded-2xl lg:rounded-r-none overflow-hidden h-full">
                  <div className="flex items-center justify-between px-4 py-3 bg-surface-100 dark:bg-surface-900/50 border-b border-surface-200 dark:border-white/5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`${quickstartTabs ? 'hidden sm:inline ml-3' : 'ml-4'} text-surface-500 text-sm font-mono`}>quickstart.py</span>
                      {quickstartTabs && (
                        <div className="ml-2 flex items-center gap-1" role="group" aria-label="Quickstart examples">
                          {Object.keys(snippets).map(name => (
                            <button
                              ...
                              className={`px-2 py-1.5 rounded-md text-xs font-medium transition-colors ${selected === name ? 'bg-libre-500/10 text-libre-700 dark:text-libre-300' : 'text-surface-500 hover:text-surface-900 dark:hover:text-white'}`}
                            >
                              {name}
                            </button>
                          ))}
                  ...
                  <pre className="text-left" aria-label={quickstartTabs ? `${selected} quickstart code` : undefined}>
                    <code className="font-mono text-sm lg:text-base">
                      {selected === 'Predict' || !quickstartTabs ? <table className="border-collapse">
                        <tbody>
                          <tr>
                          ...
                          <tr>
                            ...
                            <td><span className="text-surface-700 dark:text-surface-300">model</span> <span className="text-libre-600 dark:text-libre-400">=</span> <span className="text-emerald-600 dark:text-emerald-400">LibreYOLO</span>(<span className="token-string">&quot;LibreYOLO9t.pt&quot;</span>)</td>
                          </tr>
                          <tr>
                        ...
                        <table className="border-collapse">
                          <tbody>
                            {activeSnippet.split('\n').map((line, index) => (
                              <tr key={index}>
                                <td className="pr-4 text-right select-none text-surface-600 align-top w-6" aria-hidden="true">{index + 1}</td>
                                <td className="text-surface-700 dark:text-surface-300">
                                  {line.split(/("[^"]*"|\bfrom\b|\bimport\b)/g).map((part, i) => (
                                    <span key={i} className={part.startsWith('"') ? 'token-string' : /^(from|import)$/.test(part) ? 'token-keyword' : undefined}>{part}</span>
                                  ))}
                                </td>
                              </tr>
                            ))}
                ...
                <div className="relative bg-surface-50 dark:bg-surface-900/80 backdrop-blur-sm border border-surface-200 dark:border-white/10 rounded-2xl lg:rounded-l-none overflow-hidden h-full">
                  ...
                  <div className="p-3">
                    ...
                    <div className="mt-3 flex items-center justify-between text-xs">
                      <span className="text-emerald-400 font-mono">&#10003; {t('detected')}</span>
...
export default function LegacyHome({ quickstartTabs = false, afterSupport = null }) {
  return (
    <>
      <HeroSection quickstartTabs={quickstartTabs} />
