...
families:
  - dexined
seo_title: 'DexiNed: выделение границ со своим чекпойнтом'
description: >-
  Используйте DexiNed в LibreYOLO для плотного предсказания вероятности границ.
  Конвертируйте лицензированный чекпойнт, а затем запускайте предсказание,
  валидацию и экспорт.
lead: >-
  DexiNed (Dense Extreme Inception Network) — свёрточная сеть, которая по одному
  RGB-изображению предсказывает плотную карту вероятности границ. LibreYOLO
  использует её архитектуру только для выделения границ; чекпойнт с библиотекой
  не поставляется.
keywords:
  - DexiNed
  - Dense Extreme Inception Network
  - выделение границ python
  - детектор границ на изображении
...
DexiNed не требует установки дополнительных extra-пакетов. Всё, что он
...
LibreYOLO не поставляет чекпойнт DexiNed. Официально выпущенные веса обучены
...
`weights/convert_dexined_weights.py`: он сверяет ключи тензоров с архитектурой
...
В LibreYOLO у DexiNed один размер. Бенчмарк-стенд LibreYOLO это семейство не
...
LibreYOLO не публикует чекпойнт DexiNed. В организации LibreYOLO не размещено
ни одной копии; вместо этого конвертируйте скриптом
`weights/convert_dexined_weights.py` чекпойнт, на который у вас есть лицензия.
