...
families:
  - mobilenetv4
seo_title: 'MobileNetV4: addestra, valida ed esporta con licenza Apache-2.0'
description: >-
  Usa MobileNetV4 in LibreYOLO per la classificazione di immagini. Installa, fai
  predizioni, fine-tuning, validazione ed esportazione di LibreMobileNetV4
  small/medium/large.
lead: >-
  MobileNetV4 è un classificatore di immagini pensato per hardware mobile ed
  edge, che usa il blocco Universal Inverted Bottleneck per unificare in
  un'unica struttura esplorabile diversi blocchi mobile precedenti. LibreYOLO lo
  supporta per un solo task: la classificazione.
keywords:
  - MobileNetV4
  - MobileNetV4 conv
...
MobileNetV4 non richiede nessun extra opzionale. Tutto ciò che importa è già
...
Tre dimensioni, small/medium/large, tutte solo convoluzionali: questa famiglia
esclude le varianti ibride che aggiungono l'attenzione Mobile MQA. Scegliere una
dimensione è un semplice scambio tra numero di parametri e accuratezza. Il task
è fisso: ogni dimensione copre solo la classificazione. Il nome del file dei
pesi termina in `-cls.pt` per tutte le dimensioni, ed è quel suffisso che la
...
LoRA agisce sui componenti transformer con layer `nn.Linear` e i blocchi UIB di
questa famiglia non ne hanno.
