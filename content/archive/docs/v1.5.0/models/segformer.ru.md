...
lead: >-
  ...
  кодировок, нужных более ранним трансформерам для сегментации. LibreYOLO
  поддерживает его для одной задачи — семантической сегментации — в шести
  размерах.
keywords:
  - SegFormer
...
`(H, W)` с id классов в исходном размере изображения, а `.classes`
...
по одному классу на пиксель, а не детекции по экземплярам, которые нужно
фильтровать или очищать от дубликатов. Про источники, стриминг и обработку
...
AdamW с базовой скоростью обучения для бэкбона и в 10 раз большей
