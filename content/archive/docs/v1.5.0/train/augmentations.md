...
description: "The augmentation knobs on TrainConfig, the four pipeline shapes behind them, and the per-family table saying which knobs are used, gated or ignored."
lead: "Augmentation is configured by knobs on TrainConfig, but each model family runs its own training pipeline, and a pipeline that has no mosaic branch ignores mosaic_prob rather than approximating it."
keywords:
