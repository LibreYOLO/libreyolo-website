...
Семейства, которые не определяют семантические группы, берут консервативный
вариант по умолчанию: каждый прямой потомок модели, у которого есть хотя бы один
параметр, в порядке объявления. Обычно это короткий список, так что большое целое
число не найдёт достаточно групп:
...
- [Дистилляция](/docs/train/distillation) — другой способ перенести знания
  большой модели в запуск обучения.
