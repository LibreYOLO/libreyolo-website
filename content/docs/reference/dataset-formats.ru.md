...
verification: Повторяет docs/dataset_schema.md из репозитория libreyolo на версии v1.6.0; имена загрузчиков сверены с libreyolo/data/.
...
Датасеты RF-DETR с несколькими классами требуют `names` и могут задавать `kpt_names` для каждого класса. Пустые списки имён ключевых точек обозначают классы только с рамками. Хотя бы один класс должен иметь ключевые точки.
...
Детекция YOLO9 и RF-DETR принимает массивы `.npy` HWC с двумя плоскостями неотрицательных конечных счётчиков: сначала положительная, затем отрицательная. `input_profile` требует `format: event_histogram`, `layout: HWC`, `polarity: positive_negative`, `encoding: counts`, положительный `scale` и положительный целочисленный `window_us`. Метки используют обычные текстовые файлы детекции. См. [подготовку входа](/docs/train/event-histograms).

## Политики роботов

Задача `act` использует каталог датасета LeRobot v3 или ID датасета Hub, содержащий признаки эпизодов, камер, состояния и действий. YAML детекции не используется. См. [политики роботов](/docs/tasks/robot-policies).

## albedo

Сопоставьте `images/<split>/<name>.<image extension>` с `albedo/<split>/<name>.npy`. Целевые значения представляют собой конечные числа с плавающей точкой `(H, W, 3)` в линейном RGB в [0, 1] с теми же размерами, что и изображение. При необходимости задайте `input_dir` и `albedo_dir` как имена каталогов из одного компонента. PNG для отображения и значения sRGB не подходят как целевые значения количественной оценки альбедо.
