...
description: "Результаты LibreYOLO: рамки, маски, ключевые точки, классификация, глубина, альбедо, 3D-параллелепипеды и последовательности действий робота."
lead: Results — единый тип результата для каждого изображения у всех моделей LibreYOLO. Он содержит необязательные слоты данных,
  по одному на форму задачи, и заполняет только те, которые произвела модель.
...
verification: Имена слотов, формы, свойства и значения по умолчанию прочитаны из libreyolo/utils/results.py на версии 1.6.0.
  Семантика взята из докстрингов классов полезной нагрузки.
...
`DepthMap(data, orig_shape=None, encoding="inverse_depth")` также принимает `encoding="depth"` и `encoding="log_depth"`. Валидаторы учитывают это перед выравниванием. Значения относительной глубины по-прежнему не гарантируют метрический масштаб.
...
`plot()` визуализирует результаты всех задач. Наложения на изображения по умолчанию возвращают массивы BGR; `pil=True` запрашивает PIL. Результаты карт границ и нормалей сохраняют PIL по умолчанию.

## Boxes3D

`Boxes3D(data, orig_shape=None, intrinsics=None)` содержит строки `(N, 14)`: центр xyz, размеры wlh, кватернион wxyz, оценку ранжирования, ID класса, уверенность 2D и уверенность 3D. Координаты заданы в метрах в системе камеры: x вправо, y вниз и z вперёд; мировая система не подразумевается. Внутренние параметры камеры 3x3 относятся к исходному полю. Строки выровнены с `Results.boxes`.

## AlbedoMap

`AlbedoMap(data, orig_shape=None)` содержит линейный RGB. Преобразование для отображения создаёт sRGB; количественная валидация альбедо использует исходные линейные значения.

## Actions

`Actions(data, orig_shape=None, names=None, fps=None, instruction=None)` представляет фрагмент действий float32 `(T, D)`. `first` возвращает первую строку; срез выбирает временные шаги. Значения остаются в единицах датасета политики. `names` описывает измерения действий, `fps` частоту управления, а `instruction` текст условия.
