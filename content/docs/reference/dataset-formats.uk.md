...
keywords:
  ...
  - pose kpt_shape
last_verified: "1.6.0"
verification: Відтворює docs/dataset_schema.md у репозиторії libreyolo для v1.6.0; назви завантажувачів звірено з libreyolo/data/.
snippets:
  usage:
...
Багатокласові датасети RF-DETR потребують `names` і можуть визначати `kpt_names` для кожного класу. Порожні списки назв ключових точок позначають класи лише з рамками. Щонайменше один клас має містити ключові точки.
...
Виявлення YOLO9 і RF-DETR приймає масиви `.npy` HWC із двома площинами невід'ємних скінченних лічильників: спочатку додатна, потім від'ємна. `input_profile` потребує `format: event_histogram`, `layout: HWC`, `polarity: positive_negative`, `encoding: counts`, додатного `scale` і додатного цілого `window_us`. Мітки використовують звичайні текстові файли виявлення. Див. [підготовку вхідних даних](/docs/train/event-histograms).

## Політики роботів

Завдання `act` використовує каталог датасету LeRobot v3 або ID датасету Hub, що містить ознаки епізодів, камер, станів і дій. Воно не використовує YAML виявлення. Див. [політики роботів](/docs/tasks/robot-policies).

## albedo

Зіставляйте `images/<split>/<name>.<image extension>` з `albedo/<split>/<name>.npy`. Цільові дані є скінченними значеннями лінійного RGB із рухомою комою форми `(H, W, 3)` у [0, 1] з тими самими розмірами, що й зображення. За потреби задайте `input_dir` і `albedo_dir` як назви каталогів з одного компонента. PNG для відображення й значення sRGB не є кількісними цільовими даними альбедо.
