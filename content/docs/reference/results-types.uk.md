...
description: 'Дані результатів LibreYOLO: рамки, маски, ключові точки, класифікація, глибина, альбедо, кубоїди 3D і фрагменти дій робота.'
lead: Results є єдиним типом результату для кожного зображення в усіх моделях LibreYOLO. Він містить необов'язкові поля даних за формою завдання й заповнює лише ті, які створила модель.
keywords:
...
last_verified: "1.6.0"
verification: Назви слотів, форми, властивості та типові значення звірено з libreyolo/utils/results.py у версії v1.6.0. Семантику взято з докстрингів класів даних.
snippets:
  usage:
    - label: Python
      language: python
...
`DepthMap(data, orig_shape=None, encoding="inverse_depth")` також приймає `encoding="depth"` і `encoding="log_depth"`. Валідатори інтерпретують це перед вирівнюванням. Значення відносної глибини й далі не гарантують метричного масштабу.
...
`plot()` відображає дані кожного завдання. Накладання на зображення типово повертають масиви BGR; `pil=True` запитує PIL. Результати країв і карт нормалей зберігають типові значення PIL.

## Boxes3D

`Boxes3D(data, orig_shape=None, intrinsics=None)` містить рядки `(N, 14)`: центр xyz, розміри wlh, кватерніон wxyz, оцінку ранжування, ID класу, впевненість 2D і впевненість 3D. Координати задано в метрах у системі камери: x праворуч, y вниз і z вперед; світова система координат не мається на увазі. Внутрішні параметри 3x3 стосуються початкового полотна. Рядки узгоджено з `Results.boxes`.

## AlbedoMap

`AlbedoMap(data, orig_shape=None)` містить лінійний RGB. Перетворення для відображення створює sRGB; кількісна валідація альбедо використовує початкові лінійні значення.

## Actions

`Actions(data, orig_shape=None, names=None, fps=None, instruction=None)` представляє фрагмент дій float32 `(T, D)`. `first` повертає перший рядок; зріз вибирає часові кроки. Значення залишаються в одиницях датасету політики. `names` описує виміри дій, `fps` частоту керування, а `instruction` текст умови.
