...
verification: Повторяет docs/checkpoint_schema.md из репозитория libreyolo на версии 1.6.0, сверено с libreyolo/utils/serialization.py
  и BaseModel.save.
...
Чекпойнты позы записывают схему классов и ключевых точек. YOLO-NAS поддерживает несколько классов с одним общим скелетом; RF-DETR также поддерживает число ключевых точек для каждого класса. `nc` и `names` описывают классы. Экспорт позы для среды выполнения выдаёт `scores` с формой `[batch, anchors, nc]`.
...
RF-DETR может записывать `num_keypoints_per_class` вместе с `kpt_names`. Числа соответствуют классам датасета; ноль обозначает класс только с рамками, а предсказания дополняются до максимальной формы ключевых точек.
...
Классификаторы записывают `norm_mean`, `norm_std` и `resize_mode`. Прямоугольные поля сохраняют `imgsz_h` и `imgsz_w`. `letterbox_pad` у YOLO9 равен `topleft` или `center`; отсутствие метаданных сохраняет прежнюю геометрию с привязкой к левому верхнему углу.
...
Чекпойнты гистограмм двух полярностей сохраняют полный `input_profile`, включая формат, структуру, полярность, кодирование, масштаб и длительность окна, а также `input_initialization`. Для повторной загрузки предсказания YAML датасета не нужен; обучение и валидация отклоняют несовпадающие профили. См. [гистограммы событий](/docs/train/event-histograms).
