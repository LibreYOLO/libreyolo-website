...
last_verified: "1.6.0"
verification: Відтворює docs/checkpoint_schema.md у репозиторії libreyolo для v1.6.0; дані звірено з libreyolo/utils/serialization.py і BaseModel.save.
snippets:
  usage:
...
Контрольні точки пози записують схему класів і ключових точок. YOLO-NAS підтримує кілька класів зі спільним скелетом; RF-DETR також підтримує окрему кількість ключових точок для кожного класу. `nc` і `names` описують класи. Експорт пози для середовищ виконання видає `scores` форми `[batch, anchors, nc]`.
...
RF-DETR може записувати `num_keypoints_per_class` поруч із `kpt_names`. Кількості узгоджені з класами датасету; нуль позначає клас лише з рамками, а передбачення доповнюються до максимальної форми ключових точок.
...
Класифікатори записують `norm_mean`, `norm_std` і `resize_mode`. Прямокутні полотна зберігають `imgsz_h` і `imgsz_w`. `letterbox_pad` у YOLO9 має значення `topleft` або `center`; відсутність метаданих зберігає попередню геометрію з прив'язкою до верхнього лівого кута.
...
Власний вибір записує `fitness_source="callback"` і `best_metric_key="fitness/custom"`. Код і стан колбеку не зберігаються; ці запуски не можна відновити. Починайте новий запуск із їхніх ваг.
...
## Профілі вхідних даних

Контрольні точки гістограм двох полярностей зберігають повний `input_profile`, зокрема формат, розміщення, полярність, кодування, масштаб і тривалість вікна, а також `input_initialization`. Повторне завантаження для передбачення не потребує YAML датасету; навчання й валідація відхиляють невідповідні профілі. Див. [гістограми подій](/docs/train/event-histograms).
