...
description: >-
  Kode LibreYOLO sendiri berlisensi MIT. Kode upstream yang di-vendor dan
...
lead: >-
  ...
  yang di-vendor ke family model, dan checkpoint pretrained. Ketiganya sering
...
digunakan dalam produk komersial atau closed-source. Pertahankan baris copyright
dan teks lisensi bersama setiap salinan yang didistribusikan ulang, dan kewajiban
berakhir di sana.

Izin tersebut berhenti pada kode. File
...
menyertakan source upstream secara langsung. File yang di-vendor mempertahankan
...
Wang dan rekan penulis, implementasi SegFormer yang di-port LibreYOLO dari
...
Satu subtree bawaan tidak memakai lisensi open-source. Family DEIMv2 menyertakan
...
Dua file dalam repository memuat gambaran lengkap.
[`NOTICE`](https://github.com/LibreYOLO/libreyolo/blob/release/NOTICE)
mencantumkan setiap subtree pihak ketiga beserta path, file lisensi, dan sumber
upstream.
...
Tidak ada file bobot pretrained yang disertakan dalam package. Checkpoint
terbitan berada di Hugging Face di bawah [organisasi
LibreYOLO](https://huggingface.co/LibreYOLO), dan setiap repository memiliki
`LICENSE` serta atribusi sendiri yang mencerminkan project asal bobot.

Repository tersebut adalah sumber otoritatif untuk ketentuannya. Bukan halaman
ini, halaman model, atau ringkasan dalam source tree. Lihat
[checkpoint dan bobot](/docs/weights) untuk cara file dinamai dan lokasi
pengunduhannya.

Lisensi berbeda antar-family dan antar-file dalam satu family. Dua contoh kasus
kedua:
...
Beberapa family sama sekali tidak memiliki checkpoint yang di-host LibreYOLO,
dan halamannya menyatakan demikian pada baris Weights. SAM 3 bersifat gated di
Hugging Face berdasarkan SAM License khusus Meta dan diunduh langsung dari Meta.
Aset release MiDaS diambil dari URL resmi serta diverifikasi hash-nya, bukan
di-host ulang. Dome-DETR ditautkan ke upstream karena metadata model card-nya
...
Beberapa checkpoint torchvision tidak memiliki file lisensi sendiri. LibreYOLO
...
Tabel Checkpoints pada halaman yang sama memiliki kolom **Weights license**,
satu baris per file terbitan, sehingga family dengan ketentuan campuran
menampilkannya per file.
...
manual akan salah dalam satu release, dan kesalahan di sini berbiaya besar.

Dalam source tree, padanannya adalah `NOTICE` untuk kode bawaan,
...
Kemudian, periksa repository Hugging Face dari file persis yang akan diunduh.
Repository tersebut otoritatif dan dapat berubah tanpa perubahan halaman
dokumentasi.

## Penggunaan komersial

Kode jarang menjadi masalah. MIT, Apache-2.0, dan BSD-3-Clause semuanya
mengizinkan penggunaan komersial dan closed-source. Masing-masing meminta teks
...
nonkomersial meskipun kode di sekitarnya permisif, dan konversi file tidak
...
Jika lisensi meneruskan batasan ke karya turunan, seperti NVIDIA Source Code
...
ketentuan file yang benar-benar diunduh karena family dengan satu checkpoint
