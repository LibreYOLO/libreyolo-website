...
## Постоянные рабочие процессы не видят изменения датасета

Активные хуки `close_mosaic` или `set_epoch` должны достигать копий датасета, используемых рабочими процессами. Если собственный загрузчик с несколькими рабочими процессами сохраняет несовместимые постоянные копии, используйте `persistent_workers=False` или пересоздайте процессы после изменения. Путь без постоянных процессов, используемый по умолчанию, не затронут.
