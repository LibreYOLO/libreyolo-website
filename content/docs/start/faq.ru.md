...
## Почему имя чекпойнта не позволяет скачать его?

Используйте точное имя файла из таблицы чекпойнтов на странице модели. Имена FCN и Mask R-CNN с суффиксами задач не размещены; ошибка загрузчика перечисляет поддерживаемые формы. LingBot-Vision g не имеет опубликованного чекпойнта: используйте s, b, l или локальный чекпойнт.
