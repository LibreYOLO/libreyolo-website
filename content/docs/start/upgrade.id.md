...
Entri release lengkap tersedia pada [changelog](/docs/changelog). Bagian berikut
...
`get_download_notice()` tetap ada dan masih di-override oleh MiDaS, SegFormer,
dan YOLO9-P2.
...
Ini adalah jebakan dalam release. Baca jika memiliki hasil fine-tuning
...
melaporkan angka yang tepat, evaluasi dengan eps BN di-override ke 1e-5:
...
`base_size_repeat` sebelumnya di-hard-code ke 3 untuk setiap ukuran. Kini nilai
di-resolve per ukuran sesuai upstream: **n** berlatih pada ukuran tetap dengan
...
DEIM tetap memakai nilai 3 yang di-hard-code. Detail family tersedia pada
[D-FINE](/docs/models/d-fine).

## Perlu diketahui, tanpa tindakan

- **Hasil `imgsz` rectangle berubah karena sebelumnya salah.** Koordinat box, pengubahan ukuran mask RTMDet, rescaling YOLO-NAS, dan scaling ground truth validator kini menggunakan tinggi dan lebar per sumbu, bukan satu skalar. `imgsz` persegi tidak berubah secara bit. Inferensi atau validasi rectangle pada 1.4.0 salah skala. YOLO-NAS kini menolak `imgsz` rectangle alih-alih diam-diam menghasilkan output salah.
- **Dictionary metrik mendapatkan key baru.** `max_det`, `ar_max_det`, dan `AR_max_det` dari evaluator COCO, serta `metrics/loss` dan `metrics/loss/ce` dari FOMO. Nilai default tidak berubah, tetapi semua proses yang mengiterasi key metrik, termasuk [logger](/docs/train/loggers) kustom dan header CSV, melihat kolom baru.
...
- **Live source mengubah bentuk output JSON.** Webcam, stream RTSP, dan screen capture secara implisit mengaktifkan streaming, yang menghasilkan satu rekaman per frame, bukan satu untuk pemanggilan. [Sumber](/docs/predict/sources) tersebut baru pada 1.5.0, jadi script 1.4.0 tidak terpengaruh.
- **Ekspor ulang `rfdetr-pose` atau `yolonas-pose` ke ONNX menghasilkan nama output berbeda.** 1.4.0 keliru membaca head pose multi-tensor sebagai segmentation melalui heuristic jumlah output. File `.onnx` yang sudah ada di disk tidak berubah.
...
- **Objek konfigurasi melakukan lebih banyak validasi saat konstruksi.** `TrainConfig` mendapatkan `__post_init__`, sehingga konfigurasi yang sebelumnya sudah tidak valid kini langsung memunculkan error, bukan gagal jauh di dalam run. Serialisasi `ValidationConfig` mendapatkan key `edge_thresholds`, yang merusak round-trip ketat `ValidationConfig(**dump)` dari dump 1.4.0.
- **Nama file bobot untuk family dengan suffix task di-resolve secara berbeda.** `segformer-b0` kini di-resolve menjadi `LibreSegformerb0-sem.pt`. Ini memperbaiki 404 pengunduhan otomatis dan merusak script yang melakukan hard-code nama tanpa suffix lama.
