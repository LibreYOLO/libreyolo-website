...
Семейства эмбеддингов различаются тем, кодируют ли они целое изображение, клип или найденную область.
...
`embed_text` доступен в CLIP, SigLIP 2 и PE, у которых есть текстовые энкодеры. У DINOv2 его нет. Эмбеддинг DINOv2 обходит семантическую и классификационную головы и считывает финальный нормализованный CLS-токен при размере 224 пикселя; варианты `n`, `s`, `m` и `l` используют один энкодер DINOv2-S, поэтому все четыре возвращают `D = 384`.
...
[PE](/docs/models/pe) поддерживает эмбеддинги изображений, текста и конечных видео с `clip_frames=8` по умолчанию. [V-JEPA 2](/docs/models/vjepa2) и [LeVJEPA](/docs/models/levjepa) создают эмбеддинги клипов и предоставляют токены патчей через `embed_tokens()`. Страницы моделей описывают выбор кадров клипа и ограничения экспорта для прямого запуска в среде выполнения.
