...
[DEKR](/docs/models/dekr) забезпечує висхідне оцінювання пози кількох людей без окремого детектора людей. Підтримує інференс і валідацію, але не навчання.
...
Багатокласова поза RF-DETR використовує `kpt_names` із ключами за назвою або ID класу, щоб вибрати перші названі ключові точки для кожного класу. Порожній список позначає клас лише з рамками. Багатокласові датасети потребують `names` і щонайменше одного класу з ключовими точками. Діагностика неправильних міток указує файл, рядок і очікуваний формат `kpt_shape`.
