...
Цю задачу виконують три сімейства.
...
Вибирайте набір міток контрольної точки й підтримку навчання відповідно до вашого датасету.

[YOLO-NAS](/docs/models/yolo-nas) також підтримує навчання й інференс OBB. Попередньо навчені ваги зберігають вихідні некомерційні умови.
...
датасеті відео з БПЛА із шістьма класами транспортних
...
Навчання RF-DETR типово продовжується з
...
YOLO-NAS OBB використовує орієнтоване призначення й функції втрат, аугментацію віддзеркаленням і HSV та типове `amp=False`. Контрольні точки вибираються за `metrics/mAP50-95(OBB)`.
