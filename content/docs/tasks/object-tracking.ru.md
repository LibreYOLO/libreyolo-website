...
Остальные аргументы повторяют предсказание: `iou`, `imgsz`, `classes`, `max_det`, `vid_stride`, `show` и `save` с `output_path`. Источником может быть видео или упорядоченная последовательность изображений. Обработка результатов описана в разделе [предсказания](/docs/predict).

Последовательные кадры могут поступать из изображений, каталогов с сортировкой по имени файла, списков, кортежей и ленивых итераторов изображений. `fps=30.0` задаёт их временной шаг, а `color_format="auto"` выбирает интерпретацию входа. `vid_stride` уменьшает сохраняемую частоту до `fps / vid_stride`.

Передайте собственный экземпляр трекера через `tracker=`. Он реализует `reset()` и `update(results, image=None)` из `libreyolo.tracking.Tracker`. Запуск сбрасывает его один раз и передаёт исходный RGB-кадр PIL. Возвращаемый `track_id` должен быть одномерным целочисленным массивом или тензором, выровненным с рамками на том же бэкенде/устройстве. Для собственных экземпляров по умолчанию используется `track_conf=0.25`; настраивайте экземпляр напрямую вместо передачи аргументов трекера или `tracker_config`.
