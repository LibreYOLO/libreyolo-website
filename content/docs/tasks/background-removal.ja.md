...
BiRefNetとFeyNobgは同じ順伝播経路を共有します。
...
[BEN2](/docs/models/ben2)は、固定1024の背景除去を追加します。[ViTMatte](/docs/models/vitmatte)は、画像と、背景、未確定、前景のピクセルを表す3段階の`trimap=`を受け取ります。
...
BiRefNetとFeyNobgは固定されたネイティブの1024×1024キャンバスで実行し、マットを元画像のサイズに戻します。Swinバックボーンの相対位置テーブルはこのサイズに結び付いており、不一致の場合は例外ではなく不適切な補間が行われるため、別の解像度には対応していません。`Results.save()`はマットによる切り抜きにソース画像を使います。画像を渡さなければ`Results.path`から再読み込みします。入力ソース、ストリーミング、結果の処理については[推論](/docs/predict)を参照してください。

`Results.save()`はマットによる切り抜きをRGBAで書き出します。`plot()`は確認用の画像を描画します。BEN2はネイティブのバッチ推論に対応し、ViTMatteには単一画像のガイドが必要です。
...
これらのマットファミリーには学習実装がありません。エクスポートへの対応はファミリーごとに異なり、ViTMatteはエクスポートできません。各モデルページには、学習コードを提供するアップストリームプロジェクトと、チェックポイントを取り込む変換スクリプトが記載されています。
