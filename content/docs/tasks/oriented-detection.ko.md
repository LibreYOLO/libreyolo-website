...
실행 전에 RF-DETR 공개 체크포인트의 성격을 확인해야 합니다. DOTA는 이 작업의 기준 벤치마크지만, 해당 가중치는 DOTA로 학습하지 않았습니다. 네 가지 모두 RF-DETR 탐지 가중치로 초기화한 뒤 bike, bus, car, other_vehicle, taxi, truck의 6개 차량 클래스를 포함하는 단일 UAV 영상 데이터셋으로 파인튜닝했습니다. 모델 카드는 회전 바운딩 박스 학습 지원을 검증하면서 만든 개발용 가중치로 설명하며, 프로덕션용이나 공식 벤치마크 가중치로 해석해서는 안 된다고 명시합니다.
...
RF-DETR 학습은 기본적으로 공개 `-obb` 체크포인트에서 이어집니다. 탐지 가중치에서 시작하려면 명시적인 전이가 필요합니다. 탐지 가중치는 각도를 예측하지 않으며 `task=obb`를 전달하면 전환을 허용합니다. 이 계열의 다른 작업과 마찬가지로 `lr0`는 `1e-4` 이하로 유지합니다. RT-DETRv2의 회전 체크포인트는 파인튜닝할 수 없으므로 그대로 사용하거나 자체 레이블로 RF-DETR 모델을 학습합니다. 데이터셋, 증강, 다중 GPU, 로거는 [학습](/docs/train)을 참조하십시오.

YOLO-NAS OBB는 회전 할당과 손실, 뒤집기/HSV 증강을 사용하며, 기본값은 `amp=False`입니다. `metrics/mAP50-95(OBB)`로 체크포인트를 선택합니다.
