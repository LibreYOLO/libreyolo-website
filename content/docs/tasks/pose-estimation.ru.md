...
[DEKR](/docs/models/dekr) оценивает позы нескольких людей снизу вверх без отдельного детектора людей. Поддерживает инференс и валидацию, но не обучение.
...
RF-DETR для позы с несколькими классами использует `kpt_names` с именем или ID класса в ключах, выбирая первые именованные ключевые точки каждого класса. Пустой список обозначает класс только с рамками. Датасеты с несколькими классами требуют `names` и хотя бы один класс с ключевыми точками. Диагностика некорректных меток указывает файл, строку и ожидаемую структуру `kpt_shape`.
...
флага, переданного при обучении, так что превратить чекпойнт детекции в запуск
оценки позы одним лишь флагом не получится. Для EdgeCrafter `kpt_shape` в вашем YAML
...
площади экземпляра — на средние и крупные; корзину для мелких COCO-протокол
оценки ключевых точек не определяет. Соответствующие показатели средней полноты —
