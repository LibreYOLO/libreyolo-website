...
Следующие семейства поддерживают `normal`.

[MoGe-2](/docs/models/moge-2) специализируется на этой задаче: монокулярная геометрическая модель с одним прямым проходом и тремя размерами энкодера. Размеры s и l используют зеркала LibreYOLO; b остаётся в исходном репозитории.
...
[Marigold V2](/docs/models/marigold-v2) также оценивает нормали поверхностей. Его четырёхбитный инференс по умолчанию требует CUDA и extra `marigold`; экспорт не поддерживается.
...
Эти семейства оценки нормалей не имеют реализации обучения. Страница MoGe-2 ссылается на зафиксированные официальные чекпойнты для предсказания, валидации и экспорта.
