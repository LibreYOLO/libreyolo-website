...
実行前にRF-DETRの公開済みチェックポイントの内容を把握してください。DOTAはこのタスクの参照ベンチマークですが、これらの重みはDOTAで学習されていません。4つすべてがRF-DETRの検出用重みから初期化され、UAV映像を収録した1つのデータセットでファインチューニングされています。クラスはbike、bus、car、other_vehicle、taxi、truckの6つです。モデルカードでは、回転ボックス学習への対応を検証する間に生成した開発用の重みと説明され、製品用またはベンチマーク公式の重みとして解釈しないよう記載されています。
...
RF-DETRの学習は、デフォルトでは公開済みの`-obb`チェックポイントから続行します。検出用重みからの開始は意図的な転移です。その重みは角度を予測しないため、`task=obb`を渡すことで入れ替えを許可します。このファミリーの他タスクと同様、`lr0`は`1e-4`以下にしてください。RT-DETRv2の回転チェックポイントはファインチューニングできません。そのまま使用するか、独自ラベルでRF-DETRモデルを学習してください。データセット、データ拡張、マルチGPU、ロガーについては[学習](/docs/train)を参照してください。

YOLO-NASのOBBは回転を考慮した割り当てと損失、反転とHSVのデータ拡張を使い、デフォルトは`amp=False`です。`metrics/mAP50-95(OBB)`でチェックポイントを選択します。
