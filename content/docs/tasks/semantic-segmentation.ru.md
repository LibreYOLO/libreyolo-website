...
Обучаемые семейства: [SegFormer](/docs/models/segformer), [LingBot-Vision](/docs/models/lingbot-vision) и [DINOv2](/docs/models/dinov2). SegFormer и LingBot-Vision работают с базовым пакетом и имеют опубликованные веса. DINOv2 требует `pip install "libreyolo[rfdetr]"` и не имеет чекпойнта, размещённого LibreYOLO: он загружает исходный бэкбон, а плотная голова инициализируется случайно, поэтому это отправная точка для обучения, а не готовая модель предсказания.
...
Семантические датасеты принимают поля `(height, width)`. PP-LiteSeg и U-Net различают обрезки для обучения и прямоугольные входы для оценки. Масштабирование с обрезкой заполняет отсутствующие области игнорируемой меткой; фотометрические преобразования семейства применяются там, где настроены.
