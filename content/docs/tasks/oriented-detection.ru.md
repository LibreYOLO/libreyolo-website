...
Эту задачу поддерживают три семейства.

[RF-DETR](/docs/models/rf-detr) поддерживает обучение. Он предсказывает, обучает, валидирует и экспортирует повёрнутые рамки и имеет опубликованные чекпойнты в четырёх размерах: n, s, m и l. Ему нужен отдельный extra, `pip install "libreyolo[rfdetr]"`; лицензия и происхождение весов указаны на странице модели.
...
Выбирайте набор меток чекпойнта и поддержку обучения под свой датасет.

[YOLO-NAS](/docs/models/yolo-nas) также поддерживает обучение и инференс OBB. Предобученные веса сохраняют некоммерческие условия исходного проекта.
...
Перед запуском проверьте, что представляют собой опубликованные чекпойнты RF-DETR. Хотя DOTA служит эталонным бенчмарком этой задачи, эти веса на нём не обучались. Все четыре инициализированы весами детекции RF-DETR и дообучены на одном датасете съёмки с БПЛА с шестью классами транспорта: bike, bus, car, other_vehicle, taxi и truck. Карточки моделей описывают их как веса для разработки, полученные при проверке поддержки обучения повёрнутых рамок, и указывают, что их не следует считать весами для промышленного использования или официального бенчмарка.
...
По умолчанию обучение RF-DETR продолжается с опубликованного чекпойнта `-obb`. Начало с весов детекции требует явного переноса: эти веса не предсказывают угол, и `task=obb` разрешает замену головы. Держите `lr0` не выше `1e-4`, как и в других задачах семейства. Чекпойнты повёрнутых рамок RT-DETRv2 нельзя дообучать; используйте их как есть или обучите RF-DETR на своей разметке. Датасеты, аугментация, несколько GPU и логгеры описаны в разделе [обучения](/docs/train).

YOLO-NAS OBB использует назначение и функции потерь для повёрнутых рамок, отражения и HSV-аугментацию, а также `amp=False` по умолчанию. Чекпойнты выбираются по `metrics/mAP50-95(OBB)`.
