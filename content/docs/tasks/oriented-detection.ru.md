...
инференса, `train()` выбрасывает исключение, и переноса с его весов детекции нет —
там другой бэкбон. Трекинг и аугментация во время инференса (TTA) для повёрнутых
...
разметке с повёрнутыми рамками, а для тех категорий аэросъёмки, которыми известна DOTA,
на этих данных действительно обучены чекпойнты RT-DETRv2. `conf` и `max_det`
...
строгий и отклоняет координаты вне диапазона, при этом загрузка датасета и
валидации может сначала обрезать значения до `[0, 1]` для меток на границе
кропа, корректных во всём остальном, и всё равно затем отклонить вырожденные
рамки.
...
Родной COCO JSON тоже загружается — с отображением `annotations` из имени сплита
в JSON-файл. Аннотации читаются в порядке приоритета: поле `obb` из восьми
координат вершин в пикселях, поле `obb` вида `[cx, cy, w, h, angle]` с углом в
...
Обучение на этой задаче — это RF-DETR. По умолчанию обучение продолжается с
...
`metrics/mAP50-95` — это mean average precision, усреднённая по порогам IoU от
...
числа mAP, которые используют всю кривую точности-полноты, остаются на месте.
...
принимаются и пишут предупреждение: выгрузка предсказаний с повёрнутыми рамками
и графики валидации не реализованы.
