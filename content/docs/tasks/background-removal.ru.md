...
BiRefNet и FeyNobg используют общий прямой проход.
...
[BEN2](/docs/models/ben2) добавляет удаление фона при фиксированном размере 1024. [ViTMatte](/docs/models/vitmatte) принимает изображение и трёхуровневую `trimap=`, отмечающую фон, неизвестные пиксели и передний план.
...
BiRefNet и FeyNobg работают с фиксированным полем 1024x1024 и масштабируют альфа-маску обратно к исходному изображению. Другое разрешение не поддерживается: таблицы относительных позиций бэкбона Swin привязаны к этому размеру, а несовпадение приводит к плохой интерполяции вместо ошибки. `Results.save()` использует исходное изображение для вырезания по альфа-маске и повторно загружает его из `Results.path`, если вы его не передали. Источники, стриминг и обработка результатов описаны в разделе [предсказания](/docs/predict).

`Results.save()` сохраняет вырезанное изображение как RGBA. `plot()` визуализирует изображение для просмотра. BEN2 поддерживает нативное предсказание батчами; ViTMatte требует направляющую маску для одного изображения.
...
Эти семейства маттинга не имеют реализации обучения. Поддержка экспорта различается по семействам; ViTMatte не экспортируется. На странице каждой модели указаны исходный проект с кодом обучения и скрипт конверсии чекпойнта.
