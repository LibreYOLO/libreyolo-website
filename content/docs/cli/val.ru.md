...
| Аргумент | По умолчанию | Значение |
| --- | --- | --- |
| `classes` | `None` | Оценивать только исходные ID классов датасета через запятую (например, '0,3,5'); рамки других классов удаляются из эталонной разметки и предсказаний. По умолчанию наследует classes=, с которыми обучался чекпойнт, если они заданы |
...
| `visualize` | `False` | Рисовать каждое проверенное изображение с истинными положительными, ложными положительными и ложными отрицательными результатами в visualize/errors/ (любая ошибка) и visualize/correct/ (detect, segment; classify показывает метку и top-1) |
| `show_labels` | `True` | Имена классов на изображениях --visualize |
| `show_conf` | `True` | Оценки уверенности на изображениях --visualize |
