...
`algorithm` usa `auto` (minmax) por padrão e também aceita `minmax`, `percentile`, `mse` e `entropy`. MSE e entropy selecionam faixas com varreduras de histogramas de ativação.
