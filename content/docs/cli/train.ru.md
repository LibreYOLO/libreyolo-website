...
description: 'Обучение модели из командной строки: аргументы и значения по умолчанию, переопределения семейств и аргументы,
  которые семейство игнорирует.'
...
| Аргумент | По умолчанию | Значение |
| --- | --- | --- |
| `min_samples` | `0` | Минимальная длина эпохи для маленьких датасетов: если изображений меньше, выбрать столько примеров за эпоху с возвращением (0 = выключено) |
| `class_balanced` | `False` | Выборка с коэффициентами повторения в стиле LVIS для датасетов с длинным хвостом (по умолчанию выключена) |
| `cls_pw` | `0.0` | Степень обратной частоты при взвешивании классификации: 0 выключает, 1 полное взвешивание (средний вес класса равен 1; не сочетается с class_weights=True) |
| `class_weights` | `False` | Прежние веса функции потерь классификации, нормализованные по примерам (по умолчанию выключены) |
| `single_cls` | `False` | Обучать поддерживаемый детектор, переназначив все метки классу 0 |
| `classes` | `None` | Обучать поддерживаемый детектор только на исходных ID классов датасета через запятую (например, '0,3,5'); остальные классы отбрасываются как неразмеченные. ID сохраняются без перенумерации |
| `average_best` | `0` | Равномерно усреднить N лучших чекпойнтов по отслеживаемой метрике в weights/average.pt в конце обучения (0 = выключено) |
| `export_check` | `False` | Экспортировать ONNX до эпохи 1 и завершить запуск ошибкой, если экспорт не удался (по умолчанию выключено) |
| `precise_bn` | `0` | Пересчитать накопленную статистику BatchNorm по этому числу обучающих изображений после последней эпохи (0 = выключено) |
| `fliplr` | `None` | Вероятность горизонтального отражения (псевдоним flip_prob из экосистемы) |
| `flipud` | `0.0` | Вероятность вертикального отражения |
| `auto_augment` | `None` | Политика автоаугментации классификации: randaugment, autoaugment, augmix (по умолчанию нет) |
| `erasing` | `0.0` | Вероятность RandomErasing для классификации, 0 <= erasing < 1 |
| `cutmix` | `0.0` | Вероятность CutMix для классификации (мягкие метки) |
| `scale` | `0.5` | Диапазон площади RandomResizedCrop для классификации: нижняя граница как число или явная пара (min,max) |
| `crop_pct` | `None` | Коэффициент изменения размера перед центральной обрезкой при оценке классификации (по умолчанию нативное значение семейства) |
| `plot_samples` | `8` | Число изображений в визуализации примеров валидации: 0 отключает, -1 выводит каждое проверенное изображение (не меняет метрики) |
