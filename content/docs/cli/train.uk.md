...
description: 'Навчання моделі з командного рядка: аргументи з типовими значеннями, їх перевизначення сімействами й аргументи, які сімейство ігнорує.'
lead: >-
...
| --- | --- | --- |
| `min_samples` | `0` | Мінімальна довжина епохи для малих датасетів: якщо зображень менше, вибирати стільки зразків на епоху з поверненням (0 = вимкнено) |
| `class_balanced` | `False` | Вибірка з коефіцієнтом повторення у стилі LVIS для датасетів із довгим хвостом розподілу (типово вимкнено) |
| `cls_pw` | `0.0` | Степінь зважування класифікації за оберненою частотою: 0 вимкнено, 1 повністю (середня вага класів 1; не поєднується з class_weights=True) |
| `class_weights` | `False` | Попереднє зважування втрат класифікації з нормалізацією за зразками (типово вимкнено) |
| `single_cls` | `False` | Навчати підтримуваний детектор із перенесенням усіх міток до класу 0 |
| `classes` | `None` | Навчати підтримуваний детектор лише на цих початкових ID класів датасету, розділених комами (наприклад, '0,3,5'); усі інші класи відкидаються як нерозмічені. ID зберігаються без ущільнення |
| `average_best` | `0` | Рівномірно усереднити N найкращих контрольних точок за відстежуваною метрикою у weights/average.pt наприкінці навчання (0 = вимкнено) |
| `export_check` | `False` | Експортувати ONNX перед епохою 1 і завершити запуск помилкою, якщо експорт не вдався (типово вимкнено) |
| `precise_bn` | `0` | Повторно обчислити поточну статистику BatchNorm на цій кількості навчальних зображень після останньої епохи (0 = вимкнено) |
| `fliplr` | `None` | Ймовірність горизонтального віддзеркалення (псевдонім flip_prob в екосистемі) |
| `flipud` | `0.0` | Ймовірність вертикального віддзеркалення |
| `auto_augment` | `None` | Політика автоматичної аугментації класифікації: randaugment, autoaugment, augmix (типово відсутня) |
| `erasing` | `0.0` | Ймовірність RandomErasing для класифікації, 0 <= erasing < 1 |
| `cutmix` | `0.0` | Ймовірність CutMix для класифікації (м'які мітки) |
| `scale` | `0.5` | Діапазон площі RandomResizedCrop для класифікації: нижня межа з рухомою комою або явна пара (min,max) |
