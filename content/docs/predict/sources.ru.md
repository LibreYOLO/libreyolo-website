...
Трекинг принимает изображения, каталоги с сортировкой по имени файла, списки, кортежи и ленивые итераторы изображений как последовательные кадры. Передайте `fps=30.0`, чтобы задать время последовательности, и `color_format="auto"`, чтобы выбрать интерпретацию входа. См. [трекинг](/docs/tasks/object-tracking).
...
Если H.264 не открывается, кодирование видео переключается на доступный кодек. Переключение записывается в журнал на уровне INFO и кэшируется для кодека и размера поля только после успешного запуска другого кодека.
