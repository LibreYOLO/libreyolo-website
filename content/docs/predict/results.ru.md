...
`plot()` охватывает все результаты задач. Наложения на изображения по умолчанию возвращают непрерывные массивы HxWx3 uint8 BGR; `pil=True` запрашивает PIL. Существующие пути карт границ и нормалей сохраняют PIL по умолчанию. `orig_img` сохраняет пиксели BGR для источников в памяти и URL; локальные файлы и собранные кадры конечного видео можно открыть повторно.

Параметры управления включают `img`, `conf`, `labels`, `boxes`, `masks`, `probs`, `line_width`, `pil`, `show`, `save` и `filename`. Сохранённые изображения классификации содержат пять лучших меток. Сохранение альфа-маски записывает вырезанное изображение RGBA.
