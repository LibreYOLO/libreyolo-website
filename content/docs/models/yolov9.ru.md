...
При новом дообучении стандартной модели детекции включается ветвь PGI с `aux_weight=0.25`, используемая только при обучении. `max_labels=300`; momentum SGD прогревается от 0.8 до 0.937 за три эпохи. Старые чекпойнты с одной головой возобновляют обучение с тем же графом. Предсказание и экспорт используют основную голову. `letterbox_pad=None` наследует отметку чекпойнта: веса без отметки используют `topleft`, а новые официальные конверсии записывают `center`.

Mosaic в YOLO9 и YOLOX предпочитает изображения с разметкой, делая не более 20 попыток выбора; MixUp в YOLO9 использует ту же политику. Профили входа без RGB описаны в разделе [гистограмм событий](/docs/train/event-histograms).
...
[TFLite INT8](/docs/export/tflite) использует `int8=True` с калибровочными данными.
...
Чекпойнт для аэросъёмки с шагом 4 предсказывает классы VisDrone. Используйте лицензию, заявленную издателем в репозитории весов.
