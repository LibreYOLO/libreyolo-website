...
Для детекции, сегментации и повёрнутых рамок используется билинейное изменение размера OpenCV с плавающей точкой без сглаживания; оценка позы сохраняет сглаживание. Прямоугольный `imgsz=(height, width)` должен соответствовать сетке патчей/окон задачи. В список чекпойнтов входит детектор элементов интерфейса. [Гистограммы событий](/docs/train/event-histograms) используют записанный профиль входа.
