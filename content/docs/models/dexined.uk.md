...
families:
  - dexined
seo_title: 'DexiNed: виявлення країв із власною контрольною точкою'
description: >-
  Використовуйте DexiNed у LibreYOLO для щільного передбачення ймовірності
  країв. Перетворіть ліцензовану контрольну точку, а потім виконуйте
  передбачення, валідацію та експорт.
lead: >-
  DexiNed (Dense Extreme Inception Network) є згортковою мережею, яка передбачає
  щільну карту ймовірності країв з одного зображення RGB. LibreYOLO обгортає її
  архітектуру лише для виявлення країв; контрольна точка не постачається з
  бібліотекою.
keywords:
  - DexiNed
...
DexiNed не потребує додаткових залежностей. Усе, що вона імпортує, входить до
...
LibreYOLO постачає DexiNed в одному розмірі. Засоби бенчмарків LibreYOLO не
