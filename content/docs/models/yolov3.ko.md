...
families:
  - yolo3
seo_title: 'LibreYOLO의 YOLOv3: 예측, 검증, 내보내기'
description: >-
  LibreYOLO에서 YOLOv3를 실행합니다. tiny, base, SPP 크기가 있는 고정된 추론 전용 박물관 계열이며 퍼블릭 도메인
  라이선스로 예측, 검증, 내보내기를 지원합니다.
lead: >-
  YOLOv3는 YOLO 계열에 다중 스케일 예측과 독립 로지스틱 분류기를 추가한 Darknet-53 탐지기입니다. LibreYOLO는
  tiny, base, SPP 크기의 고정된 추론 전용 전시물로 제공합니다.
keywords:
  - YOLOv3 사용법
  - Darknet
  - Darknet-53
  ...
  predict:
    ...
    - label: SPP 크기
      ...
      code: |
        ...
        # SPP 변형은 탐지 헤드 앞에 공간 피라미드 풀링 블록을 추가하며
        # 자체 네이티브 입력 크기로 실행합니다.
...
YOLOv3에는 기본 패키지 외에 추가 항목이 필요하지 않습니다.
...
수정하면 됩니다. `conf`는 신뢰도 임계값을, `iou`는 NMS 임계값을 필터링하며 3개
헤드의 바운딩 박스를 병합하기 전에 스케일별로 적용됩니다. 소스, 스트리밍, 결과
