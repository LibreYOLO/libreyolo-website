...
DEIMは推論時に長方形の`imgsz=(height, width)`を受け付けます。DEIMv2は長方形での推論と検証の要求を明示的に拒否します。これは、長方形での学習やエクスポートへの対応を意味しません。
...
DEIMはデフォルトでAMPを有効にし、`amp_dtype="float16"`を使います。FP32で学習するには`amp=False`を渡してください。
