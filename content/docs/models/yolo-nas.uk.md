...
description: Виявлення, поза й орієнтовані рамки YOLO-NAS у LibreYOLO. Вихідні попередньо навчені ваги є некомерційними.
lead: >-
...
Завдання орієнтованих рамок повертає `result.obb`. Опублікований граф OBB використовує полотно 1024 пікселі й записаний набір міток із 18 класів.
...
Виявлення типово використовує `amp=True` з `amp_dtype="float16"`; навчання орієнтованих рамок зберігає `amp=False`. Голова OBB підтримує навчання, передбачення й валідацію, використовує аугментацію віддзеркаленням і HSV та вибирає контрольні точки за `metrics/mAP50-95(OBB)`. `load_detect_weights_for_obb()` ініціалізує її вагами виявлення.
