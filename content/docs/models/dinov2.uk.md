...
конфлікту. `output_dir=` (типове значення `None`) замінює
...
Нові запуски визначають каталог `runs/train/dinov2_exp` зі збільшуваним номером і `exist_ok=False`. `resume=True` відновлює стан тренера й зберігає вибраний каталог запуску. Класифікація підтримує `cls_pw` і `class_weights`, як описано в розділі [гіперпараметрів](/docs/train/hyperparameters).
...
Калібрування класифікації та ембедингів повторно використовує класифікаційний пайплайн моделі. Оцінювання використовує перетворення сімейства.
