...
families:
  - teed
seo_title: 'TEED: detecção de bordas, com o seu próprio checkpoint'
description: >-
  Use o TEED no LibreYOLO para predição densa de probabilidade de bordas.
  Converta um checkpoint licenciado e depois faça predições, valide e exporte.
lead: >-
  O TEED (Tiny and Efficient Edge Detector) é uma rede convolucional pequena que
...
O TEED não precisa de nenhum extra opcional. Tudo o que ele importa está na
instalação base.
...
O LibreYOLO não inclui nenhum checkpoint do TEED. Os pesos lançados
...
`weights/convert_teed_weights.py`, que confere as chaves dos tensores contra a
arquitetura de runtime antes de escrever um arquivo que o LibreYOLO consegue
...
`.binary(threshold)` retornando uma máscara booleana de bordas. Não há bounding
boxes, então `conf`, `iou` e `max_det` não têm efeito. Veja
[predição](/docs/predict) para fontes, streaming e tratamento dos resultados.

## Variantes

O TEED vem em um único tamanho no LibreYOLO. O harness de benchmark do
...
O LibreYOLO não publica nenhum checkpoint do TEED. Nada é espelhado na
organização LibreYOLO; em vez disso, converta um checkpoint para o qual você
tenha licença com `weights/convert_teed_weights.py`.
