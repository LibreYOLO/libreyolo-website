...
Предсказание для детекции принимает прямоугольный `imgsz=(height, width)`; обучение и экспорт сохраняют собственные ограничения формы.
...
Варианты `obj2coco` охватывают детекцию, сегментацию и оценку позы. Их скачивание требует явного согласия, а условия исходного проекта ограничивают коммерческое использование. Исходные чекпойнты COCO сохраняют записанное разрешение Apache-2.0.
