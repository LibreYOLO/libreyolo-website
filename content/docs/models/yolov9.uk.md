...
Нове донавчання стандартного детектора вмикає гілку PGI лише для навчання з `aux_weight=0.25`. `max_labels=300`; імпульс SGD зростає від 0.8 до 0.937 протягом трьох епох прогрівання. Старі контрольні точки з однією головою відновлюються з тим самим графом. Передбачення й експорт використовують основну голову. `letterbox_pad=None` успадковує позначку контрольної точки: ваги без позначки використовують `topleft`, а нові офіційні конверсії записують `center`.

Mosaic у YOLO9 і YOLOX надає перевагу зображенням із розміткою, виконуючи до 20 спроб вибору; MixUp у YOLO9 використовує те саме правило. Профілі вхідних даних, відмінні від RGB, описано в розділі [гістограм подій](/docs/train/event-histograms).
...
[TFLite INT8](/docs/export/tflite) використовує `int8=True` з калібрувальними даними.
...
Контрольна точка для повітряних знімків із кроком 4 передбачає класи VisDrone. Керуйтеся ліцензією, яку видавець указав у репозиторії ваг.
