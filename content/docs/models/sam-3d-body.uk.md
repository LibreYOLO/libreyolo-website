...
Автоматичне отримання потребує `libreyolo[hf]` і доступу до моделі з обмеженим доступом. Локальна контрольна точка має бути перевіреним каталогом знімка або його незміненим `model.ckpt` поруч із відповідними `model_config.yaml` і `LICENSE`. Зафіксовані хеші й дозволені переліки файлів знімка відхиляють перейменовані, змінені, пов'язані посиланнями або зайві ресурси. Ресурси MHR також зафіксовано. Не змінюйте локальний знімок, поки вихідний конструктор його читає.
