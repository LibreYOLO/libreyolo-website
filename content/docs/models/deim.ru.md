...
DEIM принимает прямоугольный `imgsz=(height, width)` для предсказания. DEIMv2 явно отклоняет прямоугольные входы при предсказании и валидации. Это не означает поддержку прямоугольных входов при обучении или экспорте.
...
DEIM по умолчанию включает AMP с `amp_dtype="float16"`. Для обучения в FP32 передайте `amp=False`.
