...
物体検出の推論は長方形の`imgsz=(height, width)`を受け付けます。学習とエクスポートには、それぞれの形状制約が引き続き適用されます。
...
`obj2coco`バリアントは、物体検出、セグメンテーション、姿勢推定に対応します。ダウンロードには明示的な同意が必要で、アップストリームの条件により商用利用が制限されます。元のCOCOチェックポイントは、記録済みのApache-2.0の許諾を維持します。
