...
description: >-
  Использование RT-DETR, RT-DETRv2 и RT-DETRv4 в LibreYOLO для детекции
  объектов, а также повёрнутые рамки в RT-DETRv2. Установка, предсказание,
  обучение, валидация и экспорт, с весами под Apache-2.0.
lead: >-
  Трансформер для детекции, сделанный под инференс в реальном времени: он
  декодирует фиксированный набор запросов, а не плотную сетку, поэтому шага NMS
  в нём нет. LibreYOLO поддерживает три его версии, которые различаются по
  загружаемому чекпойнту, а версия 2 умеет ещё и повёрнутые рамки.
keywords:
  - RT-DETR
  - RT-DETRv2
  - RT-DETRv4
  - трансформер для детекции в реальном времени
  - DETR
  ...
  predict:
    ...
    - label: Повёрнутые рамки
      ...
      code: >
        ...
        # task не нужен. Эти веса — DOTA v1.0, 15 классов аэросъёмки при 1024
        px.
        ...
        print(result.boxes.xyxy)  # охватывающие рамки, выровненные по осям
  ...
  val:
    ...
    - label: Повёрнутые рамки
      ...
      code: |
        ...
        # При валидации повёрнутых рамок сопоставление идёт по повёрнутому IoU,
        # поэтому предсказание в нужном месте, но под неверным углом — промах.
        model = LibreYOLO("LibreRTDETRv2n-obb.pt")
...
Три версии, две задачи на всех, и коды размеров не выстраиваются в один ряд.
...
Матрица описывает всю линию одной страницей: там, где три версии расходятся по
какому-то формату, в ячейке стоит самая слабая из трёх, поэтому, какую бы версию
вы ни загрузили, ничего здесь не приукрашено. Строка про повёрнутые рамки
относится только к версии 2. Там проверены ONNX и TorchScript — при FP32,
батче 1 и фиксированном холсте 1024 на 1024; OpenVINO, TensorRT и ExecuTorch
конвертируются и загружаются обратно, но не дотянули до совпадения сырых
выходов по всему набору запросов, так что верхние рамки сходятся с точностью
до доли пикселя, а хвост уплывает.
