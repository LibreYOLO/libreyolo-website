...
DEIM приймає прямокутний `imgsz=(height, width)` для передбачення. DEIMv2 явно відхиляє запити прямокутного передбачення та валідації. Це не означає підтримки прямокутного навчання чи експорту.
...
DEIM типово вмикає AMP з `amp_dtype="float16"`. Передайте `amp=False` для навчання у FP32.
