...
ошибку конфликта. `output_dir=` (по умолчанию `None`) заменяет
...
Новые запуски используют `runs/train/dinov2_exp` с увеличиваемым номером и `exist_ok=False`. `resume=True` восстанавливает состояние тренера и сохраняет выбранный каталог запуска. Классификация поддерживает `cls_pw` и `class_weights`, описанные в разделе [гиперпараметров](/docs/train/hyperparameters).
...
Калибровка классификации и эмбеддингов использует пайплайн классификации модели. Оценка использует преобразование семейства.
