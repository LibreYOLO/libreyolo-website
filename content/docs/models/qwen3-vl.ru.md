...
Установите `libreyolo[vlm-train]`, чтобы обучать LoRA-адаптеры детекции через `LibreVLM("qwen3-vl-2b").train(data=...)`. Визуальный энкодер остаётся замороженным; функция потерь на валидации выбирает лучший каталог чекпойнта. Возобновление состояния оптимизатора и валидация mAP детекции не поддерживаются. Значения по умолчанию и инструкции повторной загрузки см. в разделе [дообучения VLM](/docs/train/vlm-fine-tuning).
