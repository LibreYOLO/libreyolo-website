...
description: >-
  ...
  проприетарны и разрешены только для некоммерческого использования, и
  LibreYOLO не публикует ни один из них.
lead: >-
  Свёрточный детектор, бэкбон и neck которого вышли из поиска архитектуры в
  Deci.AI и собраны из блоков RepVGG, рассчитанных на квантизацию. Веса
  принадлежат Deci.AI, лицензированы только для некоммерческого использования,
  и LibreYOLO не публикует ни один из них.
keywords:
  - YOLO-NAS
  - YOLONAS
  - Deci AI
...
snippets:
  predict:
    - label: Python
      ...
      code: |
        ...
        # Перед скачиванием печатаются условия лицензии Deci; забирая файл, вы их принимаете.
  ...
  train:
    ...
    - label: Обучение с нуля
      ...
      code: |
        ...
        # Ни один чекпойнт Deci не задействован: модель стартует со случайных весов,
        # поэтому результат запуска происходит только из ваших данных.
  ...
  val:
    ...
    - label: Валидация на COCO
      language: bash
      code: |
        # В поставляемом YAML для COCO есть встроенный скрипт скачивания, поэтому
  ...
  export:
    ...
    - label: Использование экспортированного файла
      ...
      code: |
        ...
        # Фабрика выбирает путь по расширению файла, поэтому экспортированный артефакт
...
дообучить на скелет другого размера.

Дообучение стартует с весов Deci — именно это и покрывает лицензия Deci.
Обучение со случайно инициализированной модели не задействует чекпойнт Deci
вообще, и это третий пример кода выше.
...
придётся писать самостоятельно. Каждый формат ставится своим extra и принимает
несколько собственных аргументов. И то и другое описано на странице этого
формата.
...
pickle-файла, и его придётся скачать вручную и передать путём. Файл, уже
лежащий на диске, загружается по своему пути, без скачивания и без проверки
контрольной суммы. Это касается и файла `.pth` от Deci под его исходным именем:
...
скачивание идёт вместо этого на публичный CDN компании Deci, один раз за
процесс печатает условия Deci перед началом и сверяется с зафиксированным
SHA-256 до открытия файла.

Альтернатива — обучение со случайно инициализированной модели. Архитектура в
апстриме под Apache-2.0, а здесь под MIT, поэтому обученная так на ваших данных
модель не происходит ни от какого чекпойнта Deci.
