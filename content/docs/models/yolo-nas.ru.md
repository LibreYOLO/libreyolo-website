...
description: "Детекция, оценка позы и ориентированные рамки YOLO-NAS в LibreYOLO. Предобученные веса upstream предназначены для некоммерческого использования."
lead: >-
...
Задача повёрнутых рамок возвращает `result.obb`. Опубликованный граф OBB использует поле размером 1024 пикселя и записанный набор меток из 18 классов.
...
Для детекции по умолчанию заданы `amp=True` и `amp_dtype="float16"`; обучение повёрнутых рамок сохраняет `amp=False`. Голова OBB поддерживает обучение, предсказание и валидацию, использует отражения и HSV-аугментацию и выбирает чекпойнты по `metrics/mAP50-95(OBB)`. `load_detect_weights_for_obb()` инициализирует её весами детекции.
