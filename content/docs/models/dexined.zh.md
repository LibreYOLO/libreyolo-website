...
title: DexiNed
families: [dexined]
seo_title: "DexiNed：边缘检测，自备检查点"
description: "在 LibreYOLO 里用 DexiNed 做稠密的边缘概率预测。转换一个你持有许可的检查点，然后预测、验证并导出它。"
lead: "DexiNed（Dense Extreme Inception Network）是一个卷积网络，从一张 RGB 图像预测出稠密的边缘概率图。LibreYOLO 只为边缘检测封装了它的架构；库里不附带任何检查点。"
keywords: [DexiNed, Dense Extreme Inception Network, "边缘检测 python", BIPED, "图像边缘检测 深度学习"]
last_verified: "1.5.0"
snippets:
  predict:
    - label: Python
...
DexiNed 不需要任何可选的额外依赖。它导入的所有东西都在基础安装里。
...
LibreYOLO 不发布任何 DexiNed 检查点（checkpoint）。官方发布的权重是在 BIPED 上
训练的，而 BIPED 公布的数据集条款把用途限制在非商业目的，所以 LibreYOLO 不镜像
它们。用 `weights/convert_dexined_weights.py` 转换一个你有权使用的检查点，它会
...
DexiNed 在 LibreYOLO 里只有一个尺寸。LibreYOLO 的基准测试工具还没有测过这个家族，
所以没有可以拿来对比的公开数字。
...
LibreYOLO 不发布任何 DexiNed 检查点。LibreYOLO 组织下没有镜像任何东西；请改用
`weights/convert_dexined_weights.py` 转换一个你持有许可的检查点。
