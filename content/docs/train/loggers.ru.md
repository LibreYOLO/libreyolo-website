...
итерируемый объект, где смешано и то и другое.
...
допустимых имён. Значения, которое включает их все, нет, и CLI-флага тоже нет:
...
| Ключ | Значение |
|---|---|
| `train/loss` | средние потери обучения за эпоху |
| `train/loss/<component>` | каждая компонента потерь, о которой сообщает семейство |
...
Шаг — это номер эпохи, начиная с 1. Полностью разрешённая конфигурация обучения
логируется как параметры в начале обучения, а имя запуска по умолчанию —
...
команды установки: запросить логгер и молча не получить ничего — значит спрятать
баг.

Сбой бэкенда во время запуска работает наоборот. Первое исключение из
обработчика отключает этот логгер до конца запуска, записывает его в лог,
закрывает запуск на стороне бэкенда как неуспешный, и обучение продолжается.
Упавший сервер трекинга не стоит вам обучения.
...
Что стоит знать про отдельные бэкенды до первого запуска:

Файлы событий TensorBoard по умолчанию пишутся в `<save_dir>/tensorboard`.
Смотреть их — `tensorboard --logdir runs/train`.

В MLflow 3.x локальное файловое хранилище `./mlruns` объявлено устаревшим, и он
падает с ошибкой, если не задать `MLFLOW_ALLOW_FILE_STORE=true`. Для локального
...
`TrainStartEvent.config` — это полностью разрешённая конфигурация,
пользовательские kwargs, слитые с умолчаниями семейства, в виде отображения
только для чтения. События — замороженные датаклассы, а их отображения доступны
...
автоматическом spawn для DDP они ещё и должны быть picklable, то есть быть
классом или функцией уровня модуля, а не замыканием или лямбдой. См.
[Обучение на нескольких GPU](/docs/train/multi-gpu).

## Что любой запуск пишет в любом случае
...
| `train.log` | в реальном времени | вывод консоли запуска |

`status.json` — дешёвое чтение для скрипта или агента, который опрашивает
запуск, а атомарная запись означает, что читатель никогда не увидит недописанный
файл.
...
колонками идут компоненты потерь, метрики валидации и скорости обучения, а
заголовок расширяется, когда появляется новая колонка. При возобновлении файл
обрезается до строк перед возобновляемой эпохой, а не дублирует их.
...
`libreyolo monitor` поднимает браузерный дашборд поверх файлов выше средствами
одной только стандартной библиотеки: графики метрик, хвост лога и все
изображения валидации, с обновлением, пока запуск активен. Он работает только на
чтение и никогда не трогает процесс обучения, поэтому подключается к живому
запуску, снова открывает завершённый или разбирает упавший.
