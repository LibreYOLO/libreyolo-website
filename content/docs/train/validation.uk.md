...
Класифікація ImageFolder додає макроусереднені `metrics/precision`, `metrics/recall` і `metrics/f1` за класами, присутніми в цільових мітках валідації. Top-1 залишається типовою оцінкою якості. Виявлення також повертає `metrics/best_conf`, `metrics/best_conf_f1` і `metrics/best_conf_per_class` із ключами за назвами класів, вибираючи оптимальні за мікро-F1 пороги при IoU 0.50. Виявлення з однаковими оцінками залишаються згрупованими; за рівності вибирається вищий поріг. За відсутності додатного F1 повертається NaN. Сегментація не надає цих ключів порогів.
...
`visualize=True` записує зображення TP/FP/FN рамок із урахуванням класів для виявлення й сегментації або порівняння мітки з top-1 для класифікації ImageFolder у `visualize/errors/` і `visualize/correct/`. Зіставлення використовує IoU 0.5 і впевненість `max(0.25, conf)`. Типові значення: `visualize=False`, `show_labels=True` і `show_conf=True`. Непідтримувані завдання й валідація кліпів V-JEPA 2 відхиляють візуалізацію.

`plot_samples=8` обмежує окремий графік прикладів; 0 вимикає його, а -1 зберігає всі зображення. Це не змінює метрики чи результати візуалізації.
...
## Метрики рамок для кожного зображення

Результати виявлення й сегментації зберігають сумісність зі словниками та також надають `results.box.image_metrics`. Кожній назві файлу відповідають `precision`, `recall`, `f1`, `tp`, `fp` і `fn` за правилом зіставлення візуалізації, навіть коли візуалізацію вимкнено. Сегментація тут рахує рамки. Повторювані базові назви після першого входження використовують повні шляхи. Нульові знаменники дають 0.0. Ці записи не збираються між розподіленими рангами.
