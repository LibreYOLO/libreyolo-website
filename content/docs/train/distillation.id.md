...
description: >-
  Train a small detector against a larger teacher or a frozen DINOv2 backbone:
  the MGD, CWD and feature-MSE losses, tap points, and family support.
lead: >-
  Distillation adds a second loss term that pulls the student's intermediate
  feature maps toward a frozen teacher's. LibreYOLO taps features with forward
  hooks, so the teacher's own head and loss are never involved.
keywords:
