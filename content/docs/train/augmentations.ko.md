...
분류 ImageFolder 파이프라인에는 자체 증강 설정이 있습니다. `flip_prob`는 수평 뒤집기를 제어하며 기본값은 0.5이고, `fliplr`은 그 별칭입니다.
...
| 설정 | YOLOX 방식 | YOLO-NAS | DETR 방식 | 분류 |
|---|---|---|---|---|
| `mosaic_prob` | 사용 | 무시 | 무시 | 무시 |
| `mixup_prob` | 모자이크에 종속 | 사용 | 무시 | 무시 |
| `hsv_prob` | 사용 | 사용 | 무시 | 무시 |
| `flip_prob` | 사용 | 사용 | 사용 | 사용 |
| `flipud` | 사용 | 사용 | 무시 | 사용 |
| `degrees` | 모자이크에 종속 | 사용 | 무시 | 무시 |
| `translate` | 모자이크에 종속 | 사용 | 무시 | 무시 |
| `shear` | 모자이크에 종속 | 사용 | 무시 | 무시 |
| `perspective` | 모자이크에 종속 | 사용 | 무시 | 무시 |
| `mosaic_scale` | 모자이크에 종속 | 사용 | 무시 | 무시 |
| `mixup_scale` | 모자이크에 종속 | 사용 | 무시 | 무시 |
| `no_aug_epochs` | 사용 | 사용 | 사용 | 사용 |
...
`no_aug_epochs`는 모든 곳에서 `used`이지만 의미는 같지 않습니다. 모자이크 파이프라인에서는 마지막 에폭 동안 모자이크와 MixUp을 끕니다. DETR 방식 파이프라인에서는 광도, 축소, 크롭 증강을 중단하고 스케줄의 마지막 구간을 결정합니다. 분류 파이프라인에서는 자동 증강, 지우기, MixUp, CutMix를 비활성화하며 크롭과 뒤집기는 유지합니다.
...
CLI는 작업에 따라 `mixup`을 전달합니다. 분류는 배치 혼합에, 탐지는 `mixup_prob`에 사용합니다.

`scale=0.5`는 무작위 크롭 면적 범위 `(0.5, 1.0)`을 의미하며, 쌍을 명시하면 양쪽 경계를 설정합니다. `crop_pct=None`은 계열의 평가 크기 조정 비율을 유지합니다. 값을 지정하면 학습/검증 평가에 영향을 주지만 내보내기는 계열의 기본 전처리를 유지합니다.

CLI는 `auto_augment`, `erasing`, `cutmix`, `fliplr`, `flipud`를 제공합니다. 분류의 `mixup`은 배치 혼합을 의미하며 기본값은 0.0입니다. 수평 뒤집기의 기본값은 0.5이고 수직 뒤집기는 0.0입니다. `mixup + cutmix`는 1을 초과할 수 없습니다.

`no_aug_epochs`는 마지막 에폭 동안 자동 증강, 지우기, MixUp, CutMix를 비활성화하고 크롭과 뒤집기는 유지합니다. 분류 레시피의 마지막 구간 기본값은 0입니다.
