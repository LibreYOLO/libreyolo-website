...
У двух из них написание в CLI короче: `mosaic` отображается на `mosaic_prob`, а
`mixup` — на `mixup_prob`. Все остальные параметры пишутся одинаково в обоих
местах.

## Три состояния, а не два

Делает ли параметр хоть что-нибудь, зависит от семейства. Библиотека держит
декларативную таблицу с этим, и каждая запись в ней — одно из трёх состояний.
...
сэмплу, а аффинное преобразование и MixUp выполняет внутри ветки мозаики. Он
покрывает YOLOX, YOLOv7, YOLOv9 и его варианты E2E и P2, RTMDet, PicoDet,
...
рецепта, а не параметрами конфигурации, поэтому живыми остаются только
`flip_prob` и `no_aug_epochs`. Он покрывает D-FINE, Dome-DETR, DEIM, DEIMv2,
...
YOLO-NAS — форма сам по себе: мозаики нет вовсе, аффинное преобразование для
каждого сэмпла включено всегда, а MixUp применяется независимо, а не привязан к
мозаике. Его значение `mosaic_scale` переиспользуется как диапазон масштаба для
аффинного преобразования.

SegFormer и NAFNet выполняют каждый свой пайплайн под конкретную задачу, и
случайность в нём зафиксирована в семействе, а не настраивается. Для SegFormer
живые параметры — атрибуты класса `semantic_scale_jitter` и `semantic_hsv_prob`,
а не `mosaic_scale` и `hsv_prob`. Обрезка и отражения в NAFNet — связанные
операции над входом и целью с фиксированной вероятностью 0.5.

## Какое семейство учитывает какой параметр

Таблица ниже — это поставляемая спецификация в
...
- EC учитывает `hsv_prob`, `degrees` и `translate`, но только для `task="pose"`,
  чьё преобразование с учётом ключевых точек их читает. В путях detect и segment
  применяются фиксированные фотометрические рецепты.
- DINOv2 следует столбцу в стиле DETR для задач detect и semantic и добавляет
  набор для классификации для `task="classify"`.

`no_aug_epochs` учитывается везде, но значит он не одно и то же везде. В
...
Пайплайном классификации управляют четыре параметра, и больше ничем. Семейства
