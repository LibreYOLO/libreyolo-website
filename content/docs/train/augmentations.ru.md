...
Пайплайн классификации ImageFolder имеет собственные параметры аугментации. `flip_prob` управляет горизонтальными отражениями и по умолчанию равен 0.5; `fliplr` служит его псевдонимом.
...
`no_aug_epochs` везде имеет статус `used`, но смысл различается. В мозаичных пайплайнах он отключает мозаику и MixUp на последних эпохах. В пайплайнах в стиле DETR он останавливает фотометрические аугментации, zoom-out и обрезку и формирует хвост расписания. В пайплайнах классификации он отключает автоаугментацию, стирание, MixUp и CutMix; обрезка и отражения сохраняются.
...
CLI направляет `mixup` по задаче: классификация использует смешивание батчей, а детекция использует `mixup_prob`.

`scale=0.5` означает случайную площадь обрезки в диапазоне `(0.5, 1.0)`; явная пара задаёт обе границы. `crop_pct=None` сохраняет коэффициент изменения размера семейства для оценки; переопределение влияет на оценку при обучении и валидации, а экспорт сохраняет нативную предобработку семейства.

CLI предоставляет `auto_augment`, `erasing`, `cutmix`, `fliplr` и `flipud`. Для классификации `mixup` означает смешивание батчей и по умолчанию равен 0.0. Горизонтальные отражения по умолчанию имеют вероятность 0.5, вертикальные 0.0. `mixup + cutmix` не должно превышать 1.

`no_aug_epochs` отключает автоаугментацию, стирание, MixUp и CutMix в последних эпохах, сохраняя обрезку и отражения. В рецептах классификации этот период по умолчанию равен 0.
