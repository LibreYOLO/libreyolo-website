...
description: "Send training metrics to TensorBoard, MLflow, Weights & Biases, Comet, ClearML, Neptune or DVCLive, and write your own callback on the four training hooks."
lead: "Every trainable family emits four training events. The built-in loggers are callback objects listening to those same events, so a backend integration and a custom hook use one interface."
keywords:
