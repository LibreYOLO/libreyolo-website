...
snippets:
  ...
  doctor:
    ...
    - label: Остановка CI и на предупреждениях
      language: bash
      code: |
...
написан. Относительный сначала ищется в каталоге датасетов, затем рядом с самим
...
объектов: оно обучается как фон, а не вызывает ошибку. Строка, в которой больше
пяти полей, читается как полигон, и его рамкой становится габарит полигона,
поэтому экспорт сегментации, использованный для обучения детекции, загружается
без нареканий. Doctor сообщает, сколько строк прошло по этому пути.
...
`label_mapping` переотображает исходные id в обучающие при загрузке.
...
Восстановление сопоставляет искажённый вход с чистой целью того же разрешения
через `input_dir` и `target_dir`. Оценка глубины, нормали к поверхности и
...
Этот флаг — гейт на выполнение кода, а не на сеть. Скачивание по URL происходит
в любом случае; флаг нужен именно блокам `download: |`. CLI печатает
...
| `labels` | битые строки, id классов вне `[0, nc)`, координаты вне `[0, 1]`, рамки нулевой площади, крошечные или огромные рамки, повторяющиеся рамки, побайтово одинаковые файлы разметки |
...
Стоит знать про два поведения. С `--strict` код возврата портят не только
