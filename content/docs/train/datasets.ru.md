...
RF-DETR для позы читает `kpt_names` с ID или именем класса в ключах. Он сохраняет первые именованные строки ключевых точек каждого класса; пустой список обозначает класс только с рамками. Для позы с несколькими классами нужны `names` и хотя бы один класс с ключевыми точками.
...
Рамки с конечными координатами, пересекающие границу изображения, одинаково обрезаются при обучении и валидации. Рамки без видимой площади, с неконечными координатами и некорректные полигоны отбрасываются. ID классов вне допустимого диапазона диагностируются до построения целевых значений. `train(classes=[...])` фильтрует разметку по исходным ID классов; см. [гиперпараметры](/docs/train/hyperparameters).
